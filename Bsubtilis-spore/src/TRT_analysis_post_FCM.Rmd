...
ggsave(here("fig", "look-at-TRT-data.png"),p,
      width = 11, height = 8)
```
Mostly technical replicates are similar (=rep). Spore numbers are more variable, mainly because sometimes there replicates differ in calling spore presence (nithing vs something). Some that are off should mostly be solved by removing outliers.
...
need to deal with pipeting error that I marked by br3r,
TO DO LATER
for now, use this data
```{r}
d3 <- d3 %>% 
  mutate(bio.rep = str_replace(bio.rep, "br3r", "br3"))
```
...
```{r}
d3 %>% 
  group_by(sample.day.hr,exp.plate,t.exp,time.hr, trt, bio.rep) %>% 
